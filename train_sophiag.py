...
init_name = 'imthanhlv/vigpt2medium'
# wandb logging
...
total_bs = batch_size * gradient_accumulation_steps
...
def get_batch(split):
    ...
    x = torch.stack([torch.from_numpy((data[int(i*block_size):int((i+1)*block_size)]).astype(np.int64)) for i in ix])
    y = torch.stack([torch.from_numpy((data[int(i*block_size):int((i+1)*block_size)]).astype(np.int64)) for i in ix])
    if device_type == 'cuda':
...
if init_from == 'scratch':
    ...
    model = GPT.from_pretrained(init_from, init_name, override_args)
    ...
    if iter_num % hess_interval != hess_interval - 1:
        ...
        if master_process:
            ...
            if local_iter_num >= 5: # let the training loop settle a bit
                mfu = raw_model.estimate_mfu(batch_size * gradient_accumulation_steps, dt)
                running_mfu = mfu if running_mfu == -1.0 else 0.9*running_mfu + 0.1*mfu
            logger.info(f"iter {iter_num}: time {dt*1000:.2f}ms, mfu {running_mfu*100:.2f}%")
